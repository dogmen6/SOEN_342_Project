...
public class TempMonitor {
	...
	// replace a deployed sensor with a new deployed sensor
	public String ReplaceSensorOK(Sensor_Type sensor, Sensor_Type newSensor, Map map, Read read,
			Sensor_List sensorList) {
		for (int i = 0; i < map.mapSize(); i++) {
			if (newSensor.getSensor().equalsIgnoreCase(map.getMap_Items(i).getSensorType().getSensor())) {
				return "New sensor already exists!";
			}
		}

		for (int j = 0; j < map.mapSize(); j++) {
			if (sensor.getSensor().equalsIgnoreCase(map.getMap_Items(j).getSensorType().getSensor())) {
				map.getMap_Items(j).setSensorType(newSensor);
			}
			if (sensor.getSensor().equalsIgnoreCase(read.getRead_Items(j).getSensorType().getSensor())) {
				read.getRead_Items(j).setSensorType(newSensor);
			}
		}

		for (int k = 0; k < sensorList.SensorList_Size(); k++) {
			if (sensor.getSensor().equalsIgnoreCase(sensorList.getSensor_Type(k).getSensor())) {
				sensorList.getSensor_Type(k).setSensor(newSensor.getSensor());
				sensorList.getSensor_Type(k).setDeployed(newSensor.getDeployed());
				return "Successfully replaced the sensor!";
			}
	...
	// returns an ArrayList of location and their designated temperatures
	public ArrayList<Object> ReadCollectionOK(Map map, Read read) {
		ArrayList<Object> tempLocation = new ArrayList<Object>();

		Sensor_Type sensor1;
		Sensor_Type sensor2;

		for (int i = 0; i < map.mapSize(); i++) {
			sensor1 = map.getMap_Items(i).getSensorType();

			for (int j = 0; j < read.readSize(); j++) {
				sensor2 = read.getRead_Items(j).getSensorType();
				if (sensor1.equals(sensor2)) {
					tempLocation.add(map.getMap_Items(i).getLocationType());
					tempLocation.add(read.getRead_Items(j).getTemperatureType());
				}
			}
		}
